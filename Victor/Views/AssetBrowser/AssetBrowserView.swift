...
struct AssetBrowserView: View {
    ...
    /// Drag payload for an asset: the file itself (so a Finder drop is a real copy) plus
    /// the markdown-syntax string, so dropping back into Victor's editor still inserts
    /// `![alt](path)`. `.draggable(asset.url)` would have exported a plain `absoluteString`
    /// and lost that. Shares `FileDragItemProvider` with the sidebar's drag source.
