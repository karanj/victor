...
| PERF | User-reported typing lag, both fixes red-then-green: (1) transition guards on the dirty-state signal — per-keystroke Set.insert was firing Observation into ContentView body + NSMenu rebuild (4 withObservationTracking tests); (2) debounce moved into EditorViewModel, frontmatter serialized ONCE at fire time (user-directed design) via new AutoSaveService.scheduleImmediateSave — also fixes real data loss (frontmatter edited mid-debounce never reached disk; red test proves). Flaky debounce test strengthened w/ injected instance + explicit delay. Old actor-side debounce kept (dead from app, exercised by tests; documented). 487/487 ×2 | victor-wc1/mnu amends | **done** | this commit |

**Next up:** USER verification: typing feel in a long document (the real gate for the perf work) + toolbar crash checklist item 18. Then Phase 4 optionals (mbe, 3l6, icn) at user's discretion.
